...
type FixPos = 1 | 2 | 3 | 4 | 'TopLeft' | 'TopRight' | 'BottomRight' | 'BottomLeft'
...
const resizer = {
		...
		if (target) {
			...
			let getAngle = (target: ResizerTargetElement) => {
				...
				if (target) {
					...
					if (isNaN(angle)) {
						angle = 0
						let tmatrix = window.getComputedStyle(target as unknown as Element).transform
			...
			let handleMousedown = (e: MouseOrTouchEvent) => {
				...
				if (!target.isLocked && !target.isDisabled && target.options?.enableMove) {
					document.addEventListener('mousemove', handleMousemove)
					document.addEventListener('mouseup', handleMouseup)
					document.addEventListener('touchmove', handleMousemove)
					document.addEventListener('touchend', handleMouseup)
				}
				if (e.type === 'touchstart' && e instanceof TouchEvent && target.options?.enableMove) {
			...
			let handleMousemove = (event: MouseOrTouchEvent) => {
				...
				if (target.moveCounter === 1) {
					...
					if (target.resizer && target.resizer instanceof HTMLElement && target.resizer.showHideResizer) {
				...
				if (this.parentBounding && target.distanceParent) {
					...
					if (newX + size.width > target.distanceParent.x + this.parentBounding.width) {
						newX = target.distanceParent.x + this.parentBounding.width - size.width
					}
					...
					if (newY + size.height > target.distanceParent.y + this.parentBounding.height) {
						newY = target.distanceParent.y + this.parentBounding.height - size.height
					}
				}
			...
			target.handleMouseEnter = (e: MouseOrTouchEvent) => {
				if (this.target != e.target) {
					...
					let hoverLine = document.querySelector('.resizer-target-hover-line') as ResizerHoverLineDiv
					!hoverLine && (hoverLine = document.createElement('div') as ResizerHoverLineDiv)
					...
					hoverLine.style.transform = 'rotate(' + (tmpTarget.angle || 0) + 'deg)'
	...
	show(ele: HTMLElement) {
		...
		if (target && target.resizer) {
			if (!this.resizer) {
				let resizer = document.querySelector('.resizer-container') as ResizerDiv
				...
				resizer.showHideResizer = function (isHide, resizerHandle) {
					...
					let borderArr = resizer.querySelectorAll('*') as unknown as Array<ResizerBorderDiv | ResizerDiv>
					borderArr.forEach(x => {
				...
				let rotateXY = (cx: number, cy: number, x: number, y: number, angle: number) => {
				...
				let adjustRotateDiv = (size: Size, oldSize: Size, angle: number, fixToPos: FixPos) => {
				...
				let handleMousedown = (e: MouseOrTouchEvent) => {
					...
					if (target.angle && target.options && !target.options.resizeFromCenter) {
				...
				let handleMousemove = (event: MouseOrTouchEvent) => {
					...
					let { minWidth, minHeight, aspectRatio, maxWidth, maxHeight } = target.options
					...
					if (resizeFromCenter) {
						if (target.size.cx === undefined || target.size.cy === undefined) return
				...
				let handleMouseup = (event: MouseOrTouchEvent) => {
					...
					let target = this.target
					let resizeFromCenter = target?.options?.resizeFromCenter
					if (target && target.angle && resizer.current.actionName !== 'r-resizer' && !resizeFromCenter && this.resizer) {
